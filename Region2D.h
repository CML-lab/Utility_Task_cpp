...
public:
	...
	void SetBorderColor(GLfloat clr[]);		// Sets the polygon border color
	void SetBorderWidth(GLfloat width);		//sets the polygon border width

	int SetRectDims(GLfloat width, GLfloat height);	// Sets rectangular polygon vertices of the object (in meters) from specified width and heigh, with the bottom left corner at (0,0)
